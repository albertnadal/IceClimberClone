...
void renderMainMenuScreen(Texture2D &textureAtlas, Camera2D &staticCamera, int mountainNumber, int highScore) {
    BeginDrawing();
            ...
            BeginMode2D(staticCamera);
                    ...
                    renderNumberAtPosition(textureAtlas, 1, 1, 162, 236, false, WHITE);  // 1 text
                    ...
                    renderNumberAtPosition(textureAtlas, mountainNumber, 2, 306, 268, true, { 168, 228, 252, 255 });  // Mountain number
                    DrawTextureRec(textureAtlas, {432,336,48,16}, {162,300}, { 252, 116, 180, 255 });  // TOP (tint #fc74b4)
                    renderNumberAtPosition(textureAtlas, highScore, 6, 210, 300, true, { 252, 116, 180, 255 });  // High score number
